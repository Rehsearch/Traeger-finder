...
// ist jetzt wieder pro Träger unabhängig (kein Bezug zu Mitbewerbern), daher
// reicht ein Einzel-Aufruf pro Träger.
let mismatches = 0;
for (const r of results) {
  ...
  if (realScore !== Math.round(r.total * 10) / 10) {
    mismatches++;
    console.log(`⚠️  ABWEICHUNG bei "${r.carrier["Traeger"]}": Replik=${Math.round(r.total * 10) / 10}, echte matchCarriers()=${realScore}`);
  }
}
...
for (const [i, r] of top10.entries()) {
  console.log(`#${i + 1} ${r.carrier["Traeger"]} — Score: ${r.total}${r.rawTotal !== r.total ? ` (roh: ${r.rawTotal}, gekappt auf 0-${r.obergrenze})` : ""}`);
...
if (!gezielt) {
  ...
  console.log(`Score: ${gezielt.total}${gezielt.rawTotal !== gezielt.total ? ` (roh: ${gezielt.rawTotal}, gekappt auf 0-${gezielt.obergrenze})` : ""}`);
