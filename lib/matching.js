...
function scoreCarrier(c, a, einrichtungen = []) {
  ...
  // Kununu-Bewertung
  const kununu = parseKununuScore(c["Kununu_Score"]);
  if (kununu != null) {
    if (kununu >= 3.8) score += 15;
    else if (kununu >= 3.4) score += 8;
    else if (kununu >= 3.0) score += 0;
    else score -= 10;
  }
...
// Airtable liefert Kununu_Score als Zahl (z.B. 3.4) — für die Anzeige
// im deutschen Komma-Format formatieren.
...
function getErfuellt(c, a) {
  ...
  if (kununu != null && kununu >= 3.4) {
    reasons.push(`Gute Arbeitgeberbewertung (Kununu: ${formatKununuScore(c["Kununu_Score"])})`);
  }
