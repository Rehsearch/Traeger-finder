...
export function matchCarriers(carriers, answers) {
  ...
  return scored.map(({ carrier, score }) => ({
    ...
    hasDetailData: !!(carrier["Interne Bewertung (1-5)"] || carrier["Dienstwagen bestätigt"]),
  }));
}
...
function scoreCarrier(c, a) {
  ...
  const typ = (c["Trägertyp"] || "").toLowerCase();
  ...
  const dwTraeger = (c["Dienstwagen PDL/EL"] || c["Dienstwagen bestätigt"] || "").toLowerCase();
  ...
  if (a.versorgungsform === "stationaer" && !vf.includes("stationär")) return 0;
  if (a.versorgungsform === "ambulant"   && !vf.includes("ambulant"))  return 0;
  ...
  // Gehaltsniveau
  const gehalt = (c["Gehaltsniveau (geschätzt)"] || c["Gehaltsinfo (intern)"] || "").toLowerCase();
  score += salaryScore(gehalt, a.gehalt);

  // Region (einfaches Text-Matching)
  const region = (c["Region / Bundesländer"] || "").toLowerCase();
  ...
  const intern = parseFloat(c["Interne Bewertung (1-5)"]);
...
function cultureScore(c, grund) {
  const kultur = (c["Unternehmenskultur"] || c["Führungskultur (intern)"] || "").toLowerCase();
  const scores = {
