...
export function matchCarriers(carriers, answers, einrichtungen = []) {
  const scoredAll = carriers.map((c) => ({ carrier: c, ...scoreCarrier(c, answers, einrichtungen) }));
  ...
  // Anzeigeprozent relativ zum Punkteabstand innerhalb der Top 3 berechnen,
  // statt einer fixen Formel je Träger. Verhindert, dass mehrere Träger mit
  // unterschiedlichen Rohscores (z.B. drei AWO-Regionalverbände) am selben
  // Deckel-Wert landen und identisch angezeigt werden (siehe Testfall 2026-07-23).
  const displayScores = relativeAnzeigeScores(
    scored.map(({ score, obergrenze }) => ({ score, obergrenze }))
  );

  return scored.map(({ carrier }, i) => {
    const anzeige = displayScores[i];
    return {
      ...carrier,
      matchScore: Math.round(anzeige * 10) / 10,
      matchLabel: anzeige >= 80 ? "Sehr gute Übereinstimmung" : anzeige >= 60 ? "Gute Übereinstimmung" : "Mögliche Übereinstimmung",
...
// Bester Treffer zeigt seinen eigenen (an der Obergrenze gedeckelten) Rohscore.
// Jeder weitere Platz bekommt einen garantierten Mindestabstand zum Vorgänger
// (MIN_GAP), der sich zusätzlich am tatsächlichen Rohpunkte-Abstand orientiert
// (bis maximal MAX_GAP) — so bleiben nahe beieinanderliegende Rohscores auch
// im Anzeigewert klar unterscheidbar, statt in derselben Deckel-Sättigung zu
// verschwinden.
...
function scoreCarrier(c, a, einrichtungen = []) {
  ...
  if (a.traegerAusschluss === "kein_privat" && typ.includes("privat")) return { score: 0, obergrenze: 95 };
  if (a.traegerAusschluss === "kein_kirchlich" && typ.includes("kirchlich")) return { score: 0, obergrenze: 95 };
  if (a.traegerAusschluss === "nur_kommunal" && !typ.includes("kommunal")) return { score: 0, obergrenze: 95 };
  ...
  if (a.dienstwagen === "voraussetzung" && !dwHat) return { score: 0, obergrenze: 95 };
  ...
  // Insolvenz-Hinweis: bewusst NICHT im Score, nur informativ im
  // Beschreibungstext (siehe getNichtErfuellt).
  ...
  return { score: Math.max(0, score), obergrenze };
}
...
function getNichtErfuellt(c, a) {
  ...
  const gehalt = (c["Gehaltsniveau"] || c["Gehaltsinfo"] || "").toLowerCase();
  if (gehalt.includes("insolvenz")) {
    reasons.push("Hinweis: Trägerangaben verweisen auf ein Insolvenzrisiko — bitte vor Bewerbung selbst prüfen");
  } else if (gehaltUnterVorstellung(gehalt, a.gehalt)) {
...
function getErfuellt(c, a) {
  ...
  const gehalt = (c["Gehaltsniveau"] || c["Gehaltsinfo"] || "").toLowerCase();
  if (a.gehalt && !gehalt.includes("insolvenz") && !gehaltUnterVorstellung(gehalt, a.gehalt)) {
...
function gehaltUnterVorstellung(gehalt, wunsch) {
  if (gehalt.includes("insolvenz")) return false;
  const niveau = Object.entries(NIVEAU_MAP).find(([k]) => gehalt.includes(k))?.[1] ?? 2;
  const ziel   = WUNSCH_MAP[wunsch] ?? 2;
  return niveau < ziel;
}
