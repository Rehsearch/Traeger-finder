...
// Mathematisch hergeleitete Wertebereiche der Scoring-Formel — kein
// Schätzwert, sondern die Summe der tatsächlichen Min/Max-Werte jedes
// einzelnen Faktors. Garantiert für JEDE mögliche Eingabe-Kombination
// korrekt, nicht nur für beobachtete Testfälle.
...
export function matchCarriers(carriers, answers, einrichtungen = []) {
  ...
  return scored.map(({ carrier, score, obergrenze, geoUnbestaetigt }) => {
    const [minErwartet, maxErwartet] = gesamtGrenzen(geoUnbestaetigt);
    const anzeige = linearerDeckel(score, obergrenze, minErwartet, maxErwartet);
    return {
      ...carrier,
...
function scoreCarrier(c, a, einrichtungen = []) {
  ...
  if (a.traegerAusschluss === "kein_privat" && typ.includes("privat")) return { score: 0, obergrenze: 95, geoUnbestaetigt: false };
  if (a.traegerAusschluss === "kein_kirchlich" && typ.includes("kirchlich")) return { score: 0, obergrenze: 95, geoUnbestaetigt: false };
  if (a.traegerAusschluss === "nur_kommunal" && !typ.includes("kommunal")) return { score: 0, obergrenze: 95, geoUnbestaetigt: false };
  ...
  if (a.dienstwagen === "voraussetzung" && !dwHat) return { score: 0, obergrenze: 95, geoUnbestaetigt: false };
  ...
  return { score: Math.max(0, score), obergrenze, geoUnbestaetigt };
}
